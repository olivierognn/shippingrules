    ...
    public function applyRulesTo($rates)
    {
        ...
        foreach ($this->getValidRules() as $rule){
            ...
            foreach ($request->getAllItems() as $item){
                $item->setIsShipruleValidation(true);
            ...
            if ($rule->validateTotals($subTotals)){
                ...
                foreach ($rates as $rate){
                    ...
                    if ($rule->match($rate) &&
                        (!count(array_intersect($currentIds, $oldIds))
                            || $rule->getCalc()== Amasty_Shiprules_Model_Rule::CALC_ADD
                            || $rule->getCalc()== Amasty_Shiprules_Model_Rule::CALC_DEDUCT)) {
                        ...
                        if ($rule->getShipMin() > 0){
                            if ($rate->getPrice() + $a['fee'] < $rule->getShipMin()){
                                $a['fee'] = $rule->getShipMin() - $rate->getPrice();
                            }
                        }
                        
                        if ($rule->getShipMax() > 0){
                            if ($rate->getPrice() + $a['fee'] > $rule->getShipMax()){
                                $a['fee'] = $rule->getShipMax()  - $rate->getPrice();
                            }                        
                        }
